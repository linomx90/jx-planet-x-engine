...
#define JX_GR15_CORE_SOURCE_SHA256 \
...
jx_integrate(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(
            args,
            "iiOOOOddddddddiLLOOOOOO:integrate",
            &body_count,
            &checkpoint_count,
            &objects[0],
            &objects[1],
    ...
    if (jx_acquire_buffer(
        ...
        || jx_acquire_buffer(objects[8], &views[8], 9, 8, 1, "counters") < 0
        || jx_acquire_buffer(objects[9], &views[9], 4, 8, 1, "metrics") < 0) {
        jx_release_buffers(views, 10);
        return NULL;
    }

    Py_BEGIN_ALLOW_THREADS
    status = jx_gr15_integrate(
        body_count,
        checkpoint_count,
...
static PyMethodDef jx_methods[] = {
    {
        ...
        jx_integrate,
        METH_VARARGS,
        PyDoc_STR("Run the packaged 2--32 body GR15 numerical core.")
    },
    {
        "tableau",
        jx_tableau,
...
static struct PyModuleDef jx_module = {
    ...
    "_gr15_cpu",
    "Private compiled GR15 kernel used by the supported screening API.",
    -1,
    jx_methods,
    NULL,
    NULL,
...
PyInit__gr15_cpu(void)
{
    ...
    if (PyModule_AddIntConstant(module, "MAX_BODIES", JX_GR15_MAX_BODIES) < 0
        ...
        || PyModule_AddStringConstant(
            module, "METHOD_ID", "JX_GAUSS_RADAU15_V2") < 0
        || PyModule_AddStringConstant(
            module, "SCIENTIFIC_CLAIM_STATE", "SCREENING_ONLY") < 0
        || PyModule_AddStringConstant(
            module, "CORE_SOURCE_SHA256", JX_GR15_CORE_SOURCE_SHA256) < 0
        || PyModule_AddStringConstant(
