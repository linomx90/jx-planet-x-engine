...
#define JX_WH_MAP_KERNEL_ID "jx.wisdom-holman.native-complete-map.prototype.v2"
...
static PyObject *
jx_wh_v2_g_bundle_common(PyObject *args, int exact)
{
    double beta;
    double anomaly;
    double values[4];
    ...
    int series_terms = 0;
    int status;
    if (!PyArg_ParseTuple(args, "dd:g_bundle", &beta, &anomaly)) {
        return NULL;
    }
    status = jx_wh_loop_v2_g_bundle(
        exact,
        beta,
        anomaly,
        values,
    ...
    if (status != 0) {
        PyErr_Format(
            PyExc_FloatingPointError,
            "v2 universal-G evaluation failed with status %d",
            status);
        return NULL;
    }
...
static PyObject *
jx_wh_adaptive_g_bundle(PyObject *self, PyObject *args)
{
    (void)self;
    return jx_wh_v2_g_bundle_common(args, 0);
}


static PyObject *
jx_wh_exact_g_bundle(PyObject *self, PyObject *args)
{
    (void)self;
    return jx_wh_v2_g_bundle_common(args, 1);
}
...
jx_wh_integrate_map(PyObject *self, PyObject *args)
{
    ...
    status = jx_wh_loop_integrate_v2(
        body_count,
        checkpoint_count,
...
static PyMethodDef jx_wh_methods[] = {
    {
        "adaptive_g_bundle",
        jx_wh_adaptive_g_bundle,
        METH_VARARGS,
        PyDoc_STR(
            "adaptive_g_bundle(beta, anomaly) -> (g0,g1,g2,g3,series,terms)\n"
            "\n"
            "Evaluate the v2 early-stop recurrence with exact fallback.")
    },
    {
        "exact_g_bundle",
        jx_wh_exact_g_bundle,
        METH_VARARGS,
        PyDoc_STR(
            "exact_g_bundle(beta, anomaly) -> (g0,g1,g2,g3,series,terms)\n"
            "\n"
            "Evaluate the preserved v1 fixed-64-term fallback.")
    },
    {
        "integrate_map",
        jx_wh_integrate_map,
...
static struct PyModuleDef jx_wh_module = {
    ...
    "Private v2 native Wisdom--Holman kernel prototype; screening use only.",
    -1,
    jx_wh_methods,
    NULL,
    NULL,
...
PyMODINIT_FUNC
PyInit__wisdom_holman_cpu_v2(void)
{
#ifdef __VERSION__
