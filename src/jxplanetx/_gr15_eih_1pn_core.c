...
enum {
    JX_GR15_FORCE_NEWTONIAN = 0,
    JX_GR15_FORCE_EIH_1PN = 1
};

struct jx_gr15_force_configuration {
    int force_model;
...
int
jx_gr15_eih_1pn_tableau(double *nodes, double *weights, double *matrix)
{
...
jx_gr15_acceleration(
    ...
    const double *gm,
    struct jx_gr15_force_configuration *force_configuration,
    double *acceleration)
{
    ...
    int body;
    int source;
    if (force_configuration == NULL) {
        return JX_GR15_INPUT_DOMAIN;
    }
    if (force_configuration->force_model == JX_GR15_FORCE_EIH_1PN) {
        double observed_compactness = 0.0;
        double observed_speed_fraction_squared = 0.0;
        const int status = jx_eih_1pn_total_acceleration(
            body_count,
            position,
            velocity,
            gm,
            force_configuration->speed_of_light,
            force_configuration->maximum_compactness,
            force_configuration->maximum_speed_fraction_squared,
            acceleration,
            &observed_compactness,
            &observed_speed_fraction_squared);
        if (status == JX_EIH_1PN_INPUT_DOMAIN) {
            return JX_GR15_INPUT_DOMAIN;
        }
        if (status == JX_EIH_1PN_NONFINITE) {
            return JX_GR15_NONFINITE;
        }
        if (status == JX_EIH_1PN_FORCE_SINGULARITY) {
            return JX_GR15_FORCE_SINGULARITY;
        }
        if (status == JX_EIH_1PN_WEAK_FIELD_DOMAIN) {
            return JX_GR15_WEAK_FIELD_DOMAIN;
        }
        if (status != JX_EIH_1PN_SUCCESS) {
            return JX_GR15_NONFINITE;
        }
        force_configuration->observed_maximum_compactness = fmax(
            force_configuration->observed_maximum_compactness,
            observed_compactness);
        force_configuration->observed_maximum_speed_fraction_squared = fmax(
            force_configuration->observed_maximum_speed_fraction_squared,
            observed_speed_fraction_squared);
        return JX_GR15_SUCCESS;
    }
    if (force_configuration->force_model != JX_GR15_FORCE_NEWTONIAN) {
        return JX_GR15_INPUT_DOMAIN;
    }
...
jx_gr15_trial(
    ...
    struct jx_gr15_force_configuration *force_configuration,
    double step,
    double corrector_threshold,
    double terminal_reuse_threshold,
    ...
    int *converged_out,
    int *polynomial_predictor_used,
    double *maximum_corrector_residual,
    int64_t *terminal_force_reuses,
    int64_t *terminal_force_sweeps)
{
    const int component_count = body_count * 3;
    ...
    *converged_out = 0;
    for (iteration = 0; iteration < maximum_iterations; ++iteration) {
        ...
        *iteration_count += 1;
        if (maximum_scaled_change <= corrector_threshold) {
            *maximum_corrector_residual = fmax(
                *maximum_corrector_residual, maximum_scaled_change);
            *converged_out = 1;
            if (maximum_scaled_change
                <= terminal_reuse_threshold) {
                *terminal_force_reuses += 1;
                terminal_force_reused = 1;
            } else {
    ...
    double *force_diagnostics)
{
    ...
    double maximum_corrector_residual = 0.0;
    ...
    int64_t terminal_force_reuses = 0;
    int64_t terminal_force_sweeps = 0;
    struct jx_gr15_force_configuration force_configuration = {
        JX_GR15_FORCE_EIH_1PN,
        speed_of_light,
        maximum_compactness,
    ...
    if (input_position == NULL || input_velocity == NULL || gm == NULL
        ...
        || !isfinite(speed_of_light * speed_of_light)
        || !isfinite(maximum_compactness)
        || maximum_compactness <= 0.0 || maximum_compactness >= 1.0
    ...
    for (checkpoint_index = 1;
        ...
        while (epoch != checkpoint_epoch) {
            ...
            status = jx_gr15_trial(
                ...
                step,
                fmax(
                    convergence_factor * DBL_EPSILON,
                    epsilon * 0x1p-20),
                convergence_factor * DBL_EPSILON,
                maximum_iterations,
                history_valid,
    ...
    counters[9] = terminal_force_reuses;
    counters[10] = terminal_force_sweeps;
    metrics[0] = maximum_error;
    metrics[1] = minimum_accepted_step;
    metrics[2] = maximum_accepted_step;
    metrics[3] = proposed;
    metrics[4] = maximum_corrector_residual;
    force_diagnostics[0] = force_configuration.observed_maximum_compactness;
    force_diagnostics[1] =
        force_configuration.observed_maximum_speed_fraction_squared;
    return JX_GR15_SUCCESS;
}
