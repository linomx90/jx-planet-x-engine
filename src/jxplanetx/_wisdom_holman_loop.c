...
static int
jx_wh_g_values(double beta, double anomaly, double *values, int *used_series)
{
    ...
    int64_t *counters)
{
    ...
    int status = jx_wh_g_values(
        beta, anomaly, evaluation->g, &used_series);
    if (status != JX_WH_SUCCESS) {
        return status;
    }
    counters[JX_COUNTER_G_EVALUATIONS] += 1;
    if (used_series) {
        counters[JX_COUNTER_SERIES_TERMS] += 4 * JX_WH_LOOP_SERIES_TERMS;
    }
