...
binary64 arithmetic. It packages the qualified V3 tolerance-aware corrector,
residual-gated terminal-force reuse, and V2 cross-step predictor; it does not
compile code at runtime or import the benchmark tree.
...
class GR15Spec:
    ...
    convergence_factor: float = 256.0
...
class GR15Result:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise GR15ContractError("GR15 terminal-force accounting is inconsistent")
        if not 0.0 <= self.maximum_corrector_residual <= self.corrector_threshold:
            raise GR15ContractError("GR15 corrector residual exceeded its threshold")
        if (
...
def _native_extension() -> Any:
    ...
    try:
        ...
        raise GR15UnavailableError(
            "the packaged jxplanetx._gr15_v3_cpu extension is required; install a "
            "platform wheel or build JX from source with a C11 compiler"
        ) from exc
    if (
        _gr15_v3_cpu.MAX_BODIES != MAXIMUM_BODY_COUNT
        or _gr15_v3_cpu.STAGE_COUNT != STAGE_COUNT
        or _gr15_v3_cpu.METHOD_ID != METHOD_ID
        or _gr15_v3_cpu.SCIENTIFIC_CLAIM_STATE != SCIENTIFIC_CLAIM_STATE
        or _gr15_v3_cpu.CORE_SOURCE_SHA256 != EXPECTED_CORE_SOURCE_SHA256
    ):
        raise GR15UnavailableError("the packaged GR15 native identity is invalid")
    _NATIVE_EXTENSION = _gr15_v3_cpu
    return _NATIVE_EXTENSION
...
def _replay_digest(
    ...
    digest.update(b"jx.gr15.supported-result.v2\0")
