#!/usr/bin/env python3
"""Exact-artifact GR15 acceptance for the JX 0.6.0rc10 candidate.
...
SCHEMA = "jxplanetx.gr15-rc10.artifact-acceptance-report.v1"
PROTOCOL_SCHEMA = "jxplanetx.gr15-rc10.artifact-acceptance-protocol.v1"
BENCHMARK_ID = "jx.gr15-v3.rc10.exact-artifact-acceptance.v1"


class RC10AcceptanceError(RuntimeError):
    """The exact rc10 artifact acceptance boundary failed closed."""
...
def _path_identity(path: Path) -> dict[str, Any]:
    ...
    if path.is_symlink() or not resolved.is_file():
        raise RC10AcceptanceError(f"required artifact is not a regular file: {path}")
...
def _source_identity(path: Path, root: Path) -> dict[str, Any]:
    resolved = path.resolve(strict=True)
    if path.is_symlink() or not resolved.is_file():
        raise RC10AcceptanceError(f"required source is not regular: {path}")
    try:
        relative = resolved.relative_to(root.resolve(strict=True)).as_posix()
    except ValueError as exc:
        raise RC10AcceptanceError(f"required source escapes repository: {path}") from exc
    raw = resolved.read_bytes()
...
def _read_protocol(path: Path, root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        raise RC10AcceptanceError("protocol is not valid UTF-8 JSON") from exc
    if (
        type(protocol) is not dict
        or protocol.get("schema") != PROTOCOL_SCHEMA
        or protocol.get("benchmark_id") != BENCHMARK_ID
        or protocol.get("status") != "LOCKED_BEFORE_FIRST_EXACT_RC10_EXECUTION"
        or protocol.get("scientific_claim_state") != "SCREENING_ONLY"
        or protocol.get("package_version") != "0.6.0rc10"
...
def _verify_installed_wheel(wheel: Path, expected: dict[str, Any]) -> dict[str, Any]:
    ...
    if actual != expected:
        raise RC10AcceptanceError("wheel identity differs from the locked artifact")
    distribution = importlib.metadata.distribution("jxplanetx")
    if distribution.version != "0.6.0rc10":
        raise RC10AcceptanceError("installed distribution version is not rc10")
    checked = 0
    with zipfile.ZipFile(wheel, "r") as archive:
        records = [name for name in archive.namelist() if name.endswith(".dist-info/RECORD")]
        if len(records) != 1:
            raise RC10AcceptanceError("wheel must contain exactly one RECORD")
        ...
        for relative, digest, size in rows:
            ...
            if not digest.startswith("sha256=") or not size.isdigit():
                raise RC10AcceptanceError("wheel RECORD uses an unsupported digest")
            ...
            if (
                ...
                raise RC10AcceptanceError(f"wheel member digest failed: {relative}")
            ...
            checked += 1
    if checked < 20:
        raise RC10AcceptanceError("too few installed wheel members were verified")
    return {
        **actual,
...
def _load_base_protocol(path: Path, expected: dict[str, Any]) -> dict[str, Any]:
    ...
    if actual != expected:
        raise RC10AcceptanceError(f"base qualification protocol changed: {path.name}")
    try:
        result = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise RC10AcceptanceError(f"base protocol is invalid: {path.name}") from exc
    if result.get("scientific_claim_state") != "SCREENING_ONLY":
        raise RC10AcceptanceError("base protocol claim state changed")
    result["package_version"] = "0.6.0rc10"
    return result
...
def run(
    ...
    if sdist_record != protocol["artifacts"]["sdist"]:
        raise RC10AcceptanceError("source archive identity differs from the lock")
    ...
    if package_path.is_relative_to(source_root):
        raise RC10AcceptanceError("acceptance imported the source tree, not the wheel")
    ...
    if (
        ...
        raise RC10AcceptanceError("installed public GR15 identity changed")
...
def main() -> int:
    ...
    report["semantic_content_sha256"] = _sha256(
        b"jx.gr15-rc10.artifact-acceptance-report.v1\0" + _canonical(report)
    )
